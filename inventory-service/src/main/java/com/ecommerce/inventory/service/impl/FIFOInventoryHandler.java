...
/**
 * FIFO (First-In-First-Out) implementation of inventory handler.
 * Reserves inventory from batches with the earliest expiry dates first.
 */
@Slf4j
@Component("fifoInventoryHandler")
...
public class FIFOInventoryHandler implements InventoryHandler {
    ...
    /**
     * Reserves inventory using FIFO strategy (earliest expiry dates first).
     *
     * @param productId the ID of the product
    ...
    public boolean reserveInventory(Long productId, Integer quantity, List<Long> reservedBatchIds) {
        ...
        log.debug("Reserving {} units for product ID: {} using FIFO strategy", quantity, productId);
        List<InventoryBatch> batches = inventoryBatchRepository
